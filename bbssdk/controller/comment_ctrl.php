...
class Comment extends BaseCore
{
	...
	public function get_item()
	{
		...
		$tid = intval($_REQUEST['tid']);
		$pids = $_REQUEST['pid'];
		if(!$pids || !$tid || !$fid) return_status(403);
	...
	{		
		...
		$thread = c::t('forum_thread')->fetch_by_tid_displayorder($tid);
		if(empty($thread)) {
			return_status(405,'抱歉，指定的主题不存在或已被删除或正在被审核');
		} elseif($thread['price'] > 0 && $thread['special'] == 0 && !$_G['uid']) {
			return_status(405,"抱歉，您所在的用户组({$authAll[0]['grouptitle']})无法进行此操作");
		}
	...
	{		
		...
		if(empty($thread)){
			return_status(405,'评论已删除');
		}

		if($thread['authorid'] != $uid){
			return_status(405,'抱歉，用户不能删除其他人的评论');
		}
	...
	protected function replyItem($tid,$reppid,$thread)
	{
		...
		if($thaquote['tid'] != $_G['tid']) {
			return_status(405,"禁止引用自己和主题帖之外的帖子");
		}
	...
	protected function filter_detail($text,$tid)
	{
		...
		$newMessage = preg_replace_callback("%(src=['\"])([^'\"]*)%", 
				function($matches) use ($_G) {
					...
					{
						return $matches[1] . $_G['setting']['siteurl'] . $matches[2];
					}
					return $matches[0];
				}
			, $newMessage);
